...
def parallel_scan_tickers(
    ...
    n_workers:      defaults to SLURM_NTASKS -> SLURM_CPUS_ON_NODE -> cpu_count
    checkpoint_fn:  called in main process after each ticker completes.
                    Signature: checkpoint_fn(symbol, results, error=None)
    """
    import numpy as _np
    ...
    # Pre-convert DataFrames to numpy float32 arrays before pickling.
    # float32 halves memory; numpy arrays pickle ~10x faster than DataFrames.
    args_list = []
    ...
    bar = tqdm(
        total=n_total, desc="tickers", unit="ticker",
        dynamic_ncols=True,
        bar_format="{desc} |{bar}| {n}/{total} [{elapsed}<{remaining}, {rate_fmt}] {postfix}",
    )

    # Submit ALL futures upfront — executor keeps the pool saturated automatically.
    # Do NOT call checkpoint_fn inside the loop body for every ticker;
    # batch checkpoint writes every 50 tickers to avoid I/O serialisation.
    _pending_checkpoint: List[Tuple] = []
    CHECKPOINT_BATCH = 50

    with ProcessPoolExecutor(max_workers=n_workers, mp_context=mp_ctx) as exe:
        fut_to_sym = {exe.submit(_worker_scan_ticker, a): a[0] for a in args_list}
        ...
        for fut in as_completed(fut_to_sym):
            ...
            try:
                ...
                err = f"{e}\n{traceback.format_exc()}"
                bar.set_postfix(ticker=symbol, status="ERR", refresh=False)
                bar.update(1)
                _pending_checkpoint.append((symbol, [], err))
            else:
                ticker_results  = res.get('results', [])
                n_patterns     += len(ticker_results)
                all_results.extend(ticker_results)
                bar.set_postfix(
                    ticker=symbol, pats=n_patterns,
                    wins=res.get('wins_scanned', 0),
                    err="yes" if res.get('error') else "",
                    refresh=False,
                )
                bar.update(1)
                _pending_checkpoint.append((symbol, ticker_results, res.get('error')))

            # Flush checkpoint batch — write once per 50 tickers, not per ticker
            if len(_pending_checkpoint) >= CHECKPOINT_BATCH:
                if checkpoint_fn:
                    for sym, results, err in _pending_checkpoint:
                        checkpoint_fn(sym, results, error=err)
                _pending_checkpoint.clear()

    # Flush remainder
    if _pending_checkpoint and checkpoint_fn:
        for sym, results, err in _pending_checkpoint:
            checkpoint_fn(sym, results, error=err)
...
def parallel_scan_windows(
    ...
    """Parallelise across windows within a single ticker. Legacy path."""
    global _SHM_CACHE
    _SHM_CACHE.clear()
