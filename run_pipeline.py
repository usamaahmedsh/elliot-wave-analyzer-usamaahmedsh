...
def run_pipeline_for_interval(
    ...
    runtime_str = (f"{elapsed/60:.1f} min" if elapsed < 3600 else f"{elapsed/3600:.2f} h")

    print(f"\n{'='*70}")
    print(f"✅ {interval.upper()} complete — {runtime_str}")
    print(f"   Tickers  : {len(tickers):,} ({len(tickers)/elapsed*60:.1f} tickers/min)")
    print(f"   Patterns : {len(all_patterns):,} (avg {len(all_patterns)/max(len(tickers),1):.1f}/ticker)")
    print(f"   Avg/ticker: {avg_t:.2f}s")
    print(f"   CPU eff. : ~{cpu_eff:.0f}% across {n_workers} workers")
    if failed:
        print(f"   ⚠️ Failed : {len(failed)} tickers (see {checkpoint_mgr.failed_file})")
    print(f"{'='*70}")
